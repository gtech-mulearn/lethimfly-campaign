...
function SuccessContent() {
  ...
  const shareText = `I just committed ₹${amount} for Syam Kumar to fly for India 🇮🇳 via ${campusName || 'my campus'}! Join the #LetHimFly campaign!`;
  const shareUrl = typeof window !== 'undefined' ? window.location.origin : '';

  const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(`${shareText}\n${shareUrl}`)}`;
  const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(shareUrl)}&hashtags=LetHimFly`;

  return (
    <div className="container" style={{ paddingTop: 'var(--space-8)', paddingBottom: 'var(--space-16)', maxWidth: '600px', textAlign: 'center' }}>
      <div style={{ fontSize: '4rem', marginBottom: 'var(--space-4)', animation: 'float 3s ease infinite' }}>
        🪂
      </div>

      <h1 className="section-title">You&apos;re Committed!</h1>
      <p className="section-subtitle">
        Thank you for supporting Syam&apos;s dream to fly for India
      </p>
      ...
      <div className="card" style={{ marginBottom: 'var(--space-6)', textAlign: 'left' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 'var(--space-4)' }}>
          <div>
            ...
            <p style={{ fontFamily: 'monospace', fontSize: 'var(--text-sm)', color: 'var(--accent-primary)', wordBreak: 'break-all' }}>
              {commitmentId}
            </p>
          </div>
      ...
      {/* Next Steps */}
      <div className="card" style={{ background: 'var(--gradient-card)', borderColor: 'var(--accent-primary)' }}>
        <h3 style={{ fontSize: 'var(--text-lg)', fontWeight: 700, marginBottom: 'var(--space-3)' }}>
          ⚡ Next Step: Pay & Submit UTR
        </h3>
        <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-sm)', marginBottom: 'var(--space-4)' }}>
          Transfer ₹{amount} to the campaign account, then submit your
          transaction reference (UTR) and screenshot for verification.
        </p>
        <Link
